<template>
    ...
    <nav class="bg-sky-300 border-gray-200 px-4 lg:px-6 py-4.5 text-black p-5">
      ...
      >
        <div class="flex items-center">
          <img
            src="../assets/bettingAppLogo01.jpg"
            class="mr-1 h-6 sm:h-9 rounded-full"
            alt="Flowbite Logo"
          />
          <span
            class="self-center text-2xl font-bold whitespace-nowrap dark:text-blue"
            >Sports Betting App</span
          >
        </div>
        ...
        <div class="flex items-center lg:order-1">
          ...
          >
            <li>
              <button
                class="hover:bg-gray-600 text-white py-2 px-1 rounded"
                type="button"
                v-on:click="getUpcomingMatchList()"
              >
                Upcoming Events
              </button>
            </li>
        ...
        >
          ...
          >
            <li>
              <button
                class="bg-gray-700 hover:bg-gray-600 text-white py-2 px-1 rounded"
                type="button"
  ...
  <div
    v-for="(countryDetail, i) in countryDetailsArray"
    :key="i"
    v-show="showUpcomingMatches"
  >
    <div class="overflow-x-scroll">
      <div class="bg-green-200 h-8 flex border border-gray-300 justify-center">
        <img :src="countryDetail.flag" width="30" height="30" /><b>{{
          countryDetail.countryName
        }}</b>
      </div>
      <div class="flex">
        <div
          v-for="(matchDetail, j) in matchDetailsArray"
          :key="j"
          class="flex-shrink-0"
        >
          <div v-if="countryDetail.countryName === matchDetail.hostingCountry">
            <div class="border border-gray-600">
              <div class="flex text-sm">
                <img
                  :src="matchDetail.competitionEmblem"
                  width="30"
                  height="30"
              ...
              <div>
                ...
                <span class="text-sm font-semibold ml-1">{{
                  matchDetail.matchDateandTime.slice(0, 10)
                }}</span>
              </div>
              <div>
                <span class="text-blue-500 text-sm font-bold">Match time:</span>
                <span class="text-sm font-semibold ml-1">{{
                  matchDetail.matchDateandTime.slice(11, 16)
                }}</span>
              </div>
